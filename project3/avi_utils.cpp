...
void printOpts(AVICOMPRESSOPTIONS *opts)
{
	printf("\nCompression Options:\n");
	printf("fccType = %i\n", opts->fccType);
	printf("fccHandler = %i\n", opts->fccHandler);
	printf("dwKeyFrameEvery = %i\n", opts->dwKeyFrameEvery);
	printf("dwQuality = %i\n", opts->dwQuality);
	printf("dwBytesPerSecond = %i\n", opts->dwBytesPerSecond);
	printf("dwFlags = %i\n", opts->dwFlags);
	printf("lpFormat = %i\n", opts->lpFormat);
	printf("cbFormat = %i\n", opts->cbFormat);
	printf("lpParms = %i\n", opts->lpParms);
	printf("cbParms = %i\n", opts->cbParms);
	printf("dwInterleaveEvery = %i\n", opts->dwInterleaveEvery);
}
...
{ if (avi==NULL) return AVIERR_BADHANDLE;
  ...
  { HANDLE hf=CreateFile((LPCWSTR)src,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,0,NULL);
