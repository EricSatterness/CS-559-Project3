...
HalfCube::HalfCube() : Object()
{
	color = vec3(MakeColor(255, 69, 0, 1.0f));
}
void HalfCube::Square(glm::vec3 p1, glm::vec3 p2, glm::vec3 p3, glm::vec3 p4, vec3 color)
{
...
bool HalfCube::Initialize(float size)
{
	if (this->GLReturnedError("HalfCube::Initialize - on entry"))
		return false;
	...
	vec3 urb( size/2,  size/2,  0);
	vec3 ulb(-size/2,  size/2,  0);
	vec3 lrb( size/2, -size/2,  0);
	vec3 llb(-size/2, -size/2,  0);
	...
	if (this->GLReturnedError("HalfCube::Initialize - on exit"))
		return false;

	return true;
}

void HalfCube::TakeDown()
{
	this->vertices.clear();
	super::TakeDown();
}

void HalfCube::Draw(const mat4 & projection, mat4 modelview, const ivec2 &size)
{
