...
void Object::Draw(const mat4 & projection, mat4 modelview, const ivec2 & size)
{
	...
	currShader->Use();
	currShader->CommonSetup(0.0f, value_ptr(size), value_ptr(projection), value_ptr(modelview), value_ptr(mvp), value_ptr(nm));
	...
	{
		solid_color.Use();
		solid_color.CommonSetup(0.0f, value_ptr(size), value_ptr(projection), value_ptr(modelview), value_ptr(mvp), value_ptr(nm));
